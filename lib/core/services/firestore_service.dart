...
class FirestoreService {
  ...
  Stream<List<DishModel>> getMenuStream() {
    return _recipes.snapshots().map((snapshot) {
      ...
      // Return empty list on error to fallback to mock dishes
      return <DishModel>[];
    });
  }
  ...
  Future<void> updateRecipeItem(String id, Map<String, dynamic> updates) async {
    updates['lastUpdated'] = FieldValue.serverTimestamp();
    await _recipes.doc(id).update(updates);
  }

  Future<void> addNewDish(Map<String, dynamic> data) async {
    final newRecipeRef = _recipes.doc();
    data.remove('id'); // ID is determined by the document itself
  ...
  Future<void> submitOrder(List<DishModel> items, String userId, String tableNumber) async {
    return _db.runTransaction((transaction) async {
      ...
      // Check availability and decrement inventory
      for (var entry in ingredientsNeeded.entries) {
        ...
        try {
          ...
          // Decrement even if it goes negative for now to avoid blocking orders
          transaction.update(doc.reference, {
            'quantity': FieldValue.increment(-requiredQty)
          });
          
        } catch (e) {
          // If ingredient is missing from inventory, simply skip it instead of crashing
          print("Warning: Ingredient $name not tracked in inventory.");
        }
      }

      // Group items to match ChefScreen expectation: {'name': name, 'quantity': quantity, 'price': price}
      Map<String, Map<String, dynamic>> groupedItems = {};
      for (var dish in items) {
        if (groupedItems.containsKey(dish.name)) {
          groupedItems[dish.name]!['quantity'] = (groupedItems[dish.name]!['quantity'] as int) + 1;
        } else {
          groupedItems[dish.name] = {
            'name': dish.name,
            'quantity': 1,
            'price': dish.price,
          };
      ...
      final List<Map<String, dynamic>> chefFormattedItems = groupedItems.values.toList();
      ...
      transaction.set(orderRef, {
        'clientId': userId,
        'clientName': 'POS Staff',
        'tableNumber': tableNumber,
        'orderId': orderId,
        'items': chefFormattedItems,
        'total': items.fold(0.0, (sum, i) => sum + i.price),
        'totalPrice': items.fold(0.0, (sum, i) => sum + i.price),
        'status': 'In Kitchen',
        'timestamp': FieldValue.serverTimestamp(),
      });

      // Sync corresponding table in 'tables' collection
      final tableRef = _db.collection('tables').doc(tableNumber);
