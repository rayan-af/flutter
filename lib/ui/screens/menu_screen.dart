...
class MenuScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalItems = cartItems.fold<int>(0, (total, current) => total + current.quantity);
    ...
    return StreamBuilder<List<DishModel>>(
      ...
      builder: (context, snapshot) {
        ...
        final dishesList = snapshot.data != null && snapshot.data!.isNotEmpty 
            ? snapshot.data! 
...
class _DishItemCard extends ConsumerStatefulWidget {
  final DishModel dish;

  const _DishItemCard({required this.dish});

  @override
  ConsumerState<_DishItemCard> createState() => _DishItemCardState();
}

class _DishItemCardState extends ConsumerState<_DishItemCard> {
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => DishDetailScreen(
                  dish: widget.dish,
                  heroTag: 'menu_card_image_${widget.dish.id}',
                ),
              ),
            );
          },
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                // Image
                Hero(
                  tag: 'menu_card_image_${widget.dish.id}',
                  child: Container(
                    decoration: BoxDecoration(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        MockTranslations.getDishName(context, widget.dish),
                      ...
                      Text(
                        MockTranslations.getDishDescription(context, widget.dish),
                          ...
                          Row(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () {
                                  ref.read(ttsServiceProvider).speak(MockTranslations.getDishName(context, widget.dish));
                                },
                              ),
                              const SizedBox(width: 12),
                              GestureDetector(
                                onTap: () {
                                  ref.read(cartProvider.notifier).addItem(widget.dish);
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(AppLocalizations.of(context)!.addedToCart(MockTranslations.getDishName(context, widget.dish))),
